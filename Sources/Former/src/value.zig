...
/// We encode AST nodes and dynamic values in-language in a NaN tagged 64 bit val.
/// 0 00000001010 0000000000000000000000000000000000000000000000000001 = Float
...
// // 0x0007 if we want the type bits. This mask ensure it's NaN and it's the
// given type.
...
const Ptr = packed struct { sign: u1 = 0, 
_reserved_nan: u13 = QUIET_NAN_HEADER, _tag: Tag = Tag.ptr, pointer: u48 };

// 1 byte type header and 5 bytes of inline data. Bool, symbol, null, etc.
const Data = packed struct { sign: u1 = 0,
 _reserved_nan: u13 = QUIET_NAN_HEADER, _tag: Tag = Tag.data, 
 header: u8, data: u40 };

// 2 byte table type header. 4 byte data reference into constant table (string, class, etc.)
const TblRef = packed struct { sign: u1 = 0, 
_reserved_nan: u13 = QUIET_NAN_HEADER, _tag: Tag = Tag.tblref,
 header: u16, data: u32 };

// Unused. Data split into two equal parts.
const Split = packed struct { sign: u1 = 0, 
_reserved_nan: u13 = QUIET_NAN_HEADER, _tag: Tag = Tag.split, 
header: u24, data: u24 };
...
const Obj = packed struct { sign: u1 = 0,
 _reserved_nan: u13 = QUIET_NAN_HEADER, 
 _tag: Tag = Tag.obj, header: u8, ref: u16, data: u24 };

const Instruction = packed struct { sign: u1 = 0,
 _reserved_nan: u13 = QUIET_NAN_HEADER, 
 _tag: Tag = Tag.instruction, op: u8, register: u8, data: u32 };
// const ThreeAddr = packed struct { _reserved_nan: u13 = QUIET_NAN_HEADER, 
// _tag: Tag = Tag.instruction, op: u8, reg0: u8, mem1: u16, mem2: u16 };

// Inline string. Can store up to 8 6-bit characters encoding uppercase, 
// lowercase, digits and _.
const InlineString = packed struct { sign: u1 = 0, 
_reserved_nan: u13 = QUIET_NAN_HEADER, _tag: Tag = Tag.inline_string, data: u48 };

const InlineBitset = packed struct { sign: u1 = 0, 
_reserved_nan: u13 = QUIET_NAN_HEADER, _tag: Tag = Tag.inline_bitset, data: u48 };
...
pub fn encodeInlineString(str: []const u8) InlineString {
    ...
    for (str) |c| {
        const minCh: u6 = @truncate(switch (c) {
            ...
            'a'...'z' => c - 'a' + 10 + 26,
            '_' => 62,
            // 63 = null terminator. TODO: May revisit this. Using 0 is nicer,
            // but this representation allows easy conversion which may be moot
            // for our use-case.
            else => unreachable,
        });
        chars = (chars << 6) | minCh;
